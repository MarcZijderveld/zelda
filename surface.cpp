...
void Surface::TransparentCopyTo( Surface* a_Dst, int a_X, int a_Y )
{
	Pixel* dst = a_Dst->GetBuffer();
	Pixel* src = m_Buffer;
	if ((src) && (dst)) 
	{
		...
		if ((srcwidth > 0) && (srcheight > 0))
		{
			...
			for ( int y = 0; y < srcheight; y++ )
			{
				//Got this bit from Jan-Maarten Legerstee, my own sprite alpha was bugging out and he gave me this bit. Student nr: 130720
				for ( int x = 0; x < srcwidth; x++ )
				{
					Pixel c = a_Dst->GetBuffer()[x + (y * srcwidth)];

					if(src[x] != -16744320 && src[x] != -6710938)
					{
						dst[x] = src[x];
					}
				}
